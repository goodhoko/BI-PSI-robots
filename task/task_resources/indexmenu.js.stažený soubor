...
dTree.prototype.toString = function() {
    ...
    str += '<div id="dtree_'+this.obj+'" class="dtree '+this.config.theme+'" onclick="'+this.obj+'.divdisplay(\'r\',0);" style="overflow:';
...
dTree.prototype.getCookie = function(cookieName) {
    ...
    var pN = document.cookie.indexOf(escape(cookieName) + '=');
    if (pN != -1) {
...
dTree.prototype.openCurNS = function (max){
    var cn,match,t,i,n,cnsa,cna,cns=this.pageid;
	...
	if (cns == cn.dokuid || cns == cn.hns) {
	    ...
	    this.fajax=false;
	    if (cn.pid >= 0) addInitEvent(this.scroll("l",4,cn.pid,1));
	    break;
	}
...
dTree.prototype.show_feat = function (n){
    var div,id,dtree,dtreel,self,node=$('s'+this.obj+n);
    self=this;
    if (this.config.toc && node.className != "node") {
	div=$('t'+this.obj);
	id =(this.aNodes[n].hns) ? this.aNodes[n].hns : this.aNodes[n].dokuid;
	div.onmousedown=function (){indexmenu_ajax('req=toc&id='+id,'picker_'+self.obj,'t'+self.obj);};
...
dTree.prototype.getAjax = function(n) {
    ...
    req='req=index&id='+node.dokuid+decodeURIComponent(this.config.jsajax);
	...
	if (!isArray(ajxnodes) || ajxnodes.length < 1) {
	    ajxnodes=new Array(new Array('', 1, 0, '', 0, 1, 0));
	}
	node.ajax=false;
...
dTree.prototype.contextmenu = function(n,e) {
    var li,id,html,type,node,self,cmenu,cdtree,rmenu,X=Y=0,e;
    ...
    node =this.aNodes[n];
    self=this;
    rmenu.innerHTML='<div class="indexmenu_rmenuhead" title="'+node.name+'" onclick="'+this.obj+'.divdisplay(\'r\',0)">'+node.name+"</div>";
	...
	if (cmenu[i+1]) {
	    ...
	    html='<a href="'+eval(cmenu[i+1])+'">'+html+'</a>';
	} else {
	    type='span';
